---
name: spec-self-eval
description: Project-local Codex skill for /home/gosha/projects/audit-log-service. Self-evaluate a feature spec against the bundled review checklist (references/REVIEW.md) and the project's invariant docs (AGENTS.md, ARCHITECTURE.md, .specs/WORKFLOW.md). Writes a dated review report into the feature's .specs/<feature>/reviews/ folder. Use this skill in this project whenever the user asks to "review the spec", "evaluate requirements/design/tasks", "check the spec is ready", "run the spec review checklist", "self-review my feature spec", or any variation — and also proactively after meaningful edits to a feature's requirements.md, design.md, or tasks.md, both at the requirements+design checkpoint and at the requirements+design+tasks checkpoint defined in WORKFLOW.md. This skill OWNS spec evaluation — do not improvise a freeform review; load this skill so the report follows the project's PASS/WEAK/FAIL contract and lands in the right place.
---

# Spec self-eval

Project scope: use this skill only for `/home/gosha/projects/audit-log-service`
or an equivalent checkout of this repository. It relies on this repo's
`.specs/WORKFLOW.md`, `AGENTS.md`, and `ARCHITECTURE.md` invariants.
